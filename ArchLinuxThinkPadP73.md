...
Use `blkid` to list UUIDs of devices. Edit `/etc/default/grub` to modify variables. Append "lvm" to `GRUB_PRELOAD_MODULES`. Uncomment the `GRUB_ENABLE_CRYOTDISK=y` line. Append `cryptdevice=UUID=UUID-of-/dev/md/md127:cryptlvm root=/dev/MainVolGroup/root resume=/dev/MainVolGroup/swap ec_sys.write_support=1 nvidia-drm.modeset=1 video=1920x1080 module_blacklist=nouveau` to `GRUB_CMDLINE_LINUX_DEFAULT`. The `resume=...` part is for suspend to disk support. Install GRUB on EFI and make the config:
